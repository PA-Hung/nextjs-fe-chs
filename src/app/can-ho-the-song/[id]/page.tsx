...
import { getZaloProductById } from "@/lib/api/zalo";
...
export async function generateMetadata({ params }: ApartmentDetailPageProps): Promise<Metadata> {
  ...
  try {
    const product = await getZaloProductById(id);
...
export default async function ApartmentDetailPage({ params }: ApartmentDetailPageProps) {
  ...
  let product;
  try {
    product = await getZaloProductById(id);
  } catch (error) {
  ...
  const mainImage = product.coverImageUrl || product.images[0];
  const otherImages = product.images.filter((img) => img !== mainImage).slice(0, 4);
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:gap-12">
          ...
          <div className="flex-1 space-y-8">
            {/* Main Image */}
            <div className="relative h-[400px] w-full overflow-hidden rounded-[32px] bg-slate-100 shadow-xl sm:h-[500px] lg:h-[600px]">
              <Image
                src={mainImage}
                alt={product.name}
                fill
            ...
            {otherImages.length > 0 ? (
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {otherImages.map((image, index) => (
                  <div
                    key={index}
                    className="relative aspect-square overflow-hidden rounded-2xl bg-slate-100 shadow-md"
                  >
                    <Image
                      src={image}
            ...
            <div className="sticky top-24 rounded-[32px] bg-white/85 p-6 shadow-2xl shadow-slate-200/70 ring-1 ring-white/60 lg:p-8">
              <div className="space-y-6">
                ...
                <div className="border-t border-slate-200 pt-4">
                  <p className="mb-2 text-xs uppercase tracking-wider text-slate-500">Giá mỗi đêm</p>
